...
export default function Home() {
  ...
  const formSchema = z.object({
    name: z
      ...
      .trim()
      .toLowerCase()
      .min(1, {message: "Debe definir un nombre para el producto"})
      .refine((val) => !products.some((product) => product.name === val), {
        message: "Este producto ya existe",
      }),
    price: z.coerce
      .number({invalid_type_error: "Debe ingresar un numero"})
      .positive({message: "El valor debe ser positivo"}),
    amount: z.coerce
      .number()
      .int({message: "El valor debe ser un numero entero"})
      .positive({message: "El valor debe ser positivo"})
      .optional()
      .or(z.string().max(0)),
  });
    ...
    <main className="max-w-[800px] h-[100svh] mx-auto grid grid-rows-[1fr_auto] overflow-hidden justify-items-center">
      {products.length > 0 ? (
        <ul className="w-full flex flex-col gap-4 overflow-auto py-2 px-3">
          {products.map((product) => (
            <li
              key={product.name}
              className="capitalize bg-[whitesmoke] rounded-[6px] py-6 px-4 flex justify-between"
            >
              <div>
                <p>{product.name}</p>
              ...
              <div>
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button variant="outline" className="bg-[whitesmoke]">
                      X
                    </Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent className="w-[350px]">
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction onClick={() => removeProduct(product.name)}>
                        Continuar
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
          ...
          <li className="flex w-full justify-end">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent className="w-[350px]">
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancelar</AlertDialogCancel>
                  <AlertDialogAction onClick={() => removeAllProducts()}>
                    Continuar
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
      ...
      <div className="w-full flex justify-between items-center relative flex-cols py-4 bg-[whitesmoke] px-8 rounded-[6px]">
        ...
        <Dialog open={IsModalOpen} onOpenChange={toggleModal}>
          <DialogTrigger className="w-[60px] h-[60px] absolute top-[-20px] rounded-full bg-white left-[calc(50%-30px)] ">
            +
          </DialogTrigger>
